...
ggplot(data=hoteles, mapping = aes(rank_bk,rank_ta,color=prices_bk)) +
  geom_point() + 
  scale_color_paletteer_c(cpalette,direction=1)+
  labs(x="Posición en Booking", y="Posición en TripAdvisor",
...
ggplot(data=hoteles, mapping = aes(rank_bk,rank_ta, color=city_ta, shape=city_ta)) +
  geom_point(size=3.5) + 
  scale_colour_paletteer_d(palette, direction=-1) +
  labs(x="Posición en Booking", y="Posición en TripAdvisor",
...
ggplot(data=hoteles)+
  geom_boxplot(mapping = aes(x=prices_bk), color=main, fill=lmain,) + 
...
ggplot(data=hoteles)+
  geom_boxplot(mapping = aes(x=prices_ta), color=main, fill=lmain,) + 
  labs(x="Precio por noche(MXN)",
       title="Distribución de precios por noche en\nBooking",
       caption="Fuente: Elaboración propia (2022)") +
...
ggplot(tmp, aes(x = "", y = count, fill = as.factor(inclusion))) +
  ...
  scale_fill_manual(values=c("violetred4","bisque2"),labels=c('No','Sí'),"") +
